class Importers::Species::Taxonomies < Importers::Base
  def initialize
    @species = {}
    @taxon_classes = {}
    @taxon_orders = {}
  ...
  def allowed_taxo_class? taxo_class
    Import.configuration['taxonomic_classes'].include?(taxo_class)
  end

  def taxon_class class_name
    @taxon_classes[class_name] ||= TaxonomicClass.find_or_create_by(name: class_name)
  end

  def taxon_order order_name, taxon_class
    @taxon_orders[order_name] ||= TaxonomicOrder.find_or_create_by(
      name: order_name,
  ...
  def source_file_path
    Rails.root.join('lib/data/species/cc_vulnerable_species.csv')
  end
