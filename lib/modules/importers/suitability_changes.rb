class Importers::SuitabilityChanges < Importers::Base
  extend Memoist
  SPECIES_RANGE = (5..-3)
  ...
  def populate_species record, taxonomic_class_id
    @all_species = record[SPECIES_RANGE].map do |species_name|
      ...
      Species.find_or_create_by(name: name) do |species|
        species.taxonomic_order_id = TaxonomicOrder.find_or_create_by(
          name: 'Not Available',
          taxonomic_class_id: taxonomic_class_id
        ).id
      end
    end
  end
  ...
  memoize def files
    Dir['lib/data/suitability_changes/*']
  end
