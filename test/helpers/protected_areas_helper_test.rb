...
class ProtectedAreasHelperTest < ActionView::TestCase
  ...
  def test_iucn_red_list_icon
    value = "EN"
    assert_equal %Q(#{value} <i class="fa fa-circle orange"></i>), iucn_value_icon(value)
  end

  def test_suitability_value_icon
    value = "Inc"
    assert_equal %Q(#{value.upcase} <i class="fa fa-arrow-circle-up green"></i>), suitability_value_icon(value)
  end

  def test_traits_to_return_list
    taxonomic_class = FactoryGirl.build(:taxonomic_class, name: "Amphibian")
    taxonomic_order = FactoryGirl.build(:taxonomic_order, taxonomic_class: taxonomic_class)
    species = FactoryGirl.build(:species, taxonomic_order: taxonomic_order, adaptability: "1")
  ...
  def high_priority_warning_to_return_div_if_high_priority
    protected_area = FactoryGirl.create(:protected_area_with_high_priority)
    assert_equal %Q(
