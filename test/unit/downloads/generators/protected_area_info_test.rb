...
class DownloadsGeneratorsProtectedAreaInfoTest < ActiveSupport::TestCase
  test '::generate, given a ProtectedArea, returns a path to the created csv' do
    ...
    CSV.stubs(:open)
    assert_equal expected_path, Downloads::Generators::ProtectedAreaInfo.generate(pa)
  end
  ...
  test '::generate, given a ProtectedArea, creates a csv file' do
    ...
    csv_mock = mock
    csv_mock.expects(:<<).with(Downloads::Generators::ProtectedAreaInfo::HEADERS)
    csv_mock.expects(:<<).with([123, 'Gola', 'GHA', 'National Park', 'IV', 1000, 100, 10, false])
    CSV.stubs(:open).yields(csv_mock)

    Downloads::Generators::ProtectedAreaInfo.generate(pa)
  end
end
